...
async function setupSipTrunk(apiService, name, domain, spinner, provider, configKey) {
  ...
  try {
    ...
    spinner.succeed(
      chalk.green(`SIP trunk "${name}" created successfully for domain ${selectedDomain}`)
    );
...
async function selectDomain(specifiedDomain, spinner) {
  ...
  if (!domainConfig) {
    ...
    console.log(
      chalk.yellow(
        `Use 'cx-vcc configure --domain ${selectedDomain} --apikey YOUR_API_KEY' to add it.`
      )
    );
    process.exit(1);
  }
  ...
  if (!domainConfig.inboundSipUri) {
    console.error(
      chalk.red(`Domain ${selectedDomain} does not have an inbound SIP URI configured.`)
    );
    console.log(
      chalk.yellow(
        `Please reconfigure the domain: 'cx-vcc configure --domain ${selectedDomain} --apikey YOUR_API_KEY'`
      )
    );
    process.exit(1);
  }
...
async function serviceCommand(options) {
  ...
  let providerEntry;
  try {
    providerEntry = getProviderConfig(provider);
  } catch (err) {
    console.error(chalk.red(err.message));
    console.log(
      chalk.yellow(`Currently supported providers: ${getSupportedProviders().join(', ')}`)
    );
    process.exit(1);
  }

  await configureServiceProvider(options, providerEntry.Service, providerEntry.configKey);
}
