...
async function syncCommand(options) {
  ...
  if (
    provider &&
    (provider.toLowerCase() === '11labs' || provider.toLowerCase() === 'elevenlabs')
  ) {
  ...
  if (domain && provider) {
    console.log(
      chalk.blue.bold(`Synchronizing ${displayProvider} configuration for domain ${domain}...`)
    );
  } else if (domain) {
    console.log(chalk.blue.bold(`Synchronizing all providers for domain ${domain}...`));
  } else if (provider) {
    console.log(
      chalk.blue.bold(`Synchronizing ${displayProvider} configuration for all domains...`)
    );
  } else {
    console.log(
      chalk.blue.bold('Synchronizing local configuration with all remote service providers...')
    );
  }
  ...
  if (shouldSyncProvider('vapi') && config.vapi && config.vapi.apiKey) {
    const changes = await syncProviderNumbers(
      'VAPI',
      config.vapi,
      async () => {
  ...
  if (
    (shouldSyncProvider('11labs') || shouldSyncProvider('elevenlabs')) &&
    config.elevenlabs &&
  ...
  if (totalChanges > 0) {
    console.log(
      chalk.green(`Sync complete. ${totalChanges} phone numbers removed from local configuration.`)
    );
  } else {
    console.log(
      chalk.green('Sync complete. Local configuration is already in sync with remote services.')
    );
  }
}
...
async function syncProviderNumbers(providerName, providerConfig, fetchRemoteNumbers, domainFilter) {
  ...
  if (domainFilter) {
    ...
    if (config.domains[domainFilter]) {
      ...
      const providerKeys =
        providerName.toLowerCase() === '11labs'
          ? ['elevenlabs', '11labs']
          : [providerName.toLowerCase()];

      for (const providerKey of providerKeys) {
        if (
          domainConfig[providerKey] &&
  ...
  if (!hasPhoneNumbers) {
    console.log(
      chalk.yellow(
        `No ${providerName} phone numbers in local configuration${domainFilter ? ` for domain ${domainFilter}` : ''}.`
      )
    );
    return 0;
  }
  ...
  console.log(
    chalk.cyan(
      `Found ${localPhoneNumbers.length} ${providerName} phone numbers${domainFilter ? ` for domain ${domainFilter}` : ''} in local configuration:`
    )
  );
  ...
  try {
    ...
    if (Array.isArray(remoteData)) {
      ...
      if (providerName === 'VAPI') {
        ...
        remotePhoneNumbers = remoteData.map((item) => item.phone_number || item.phoneNumber || '');
      } else if (providerName === '11Labs') {
        remotePhoneNumbers = remoteData.map(
          (item) => item.phone_number || item.phoneNumber || item.number || ''
        );
      }
    ...
    spinner.succeed(
      `Found ${remotePhoneNumbers.length} ${providerName} phone numbers in remote service${domainFilter ? ` (not filtered by domain)` : ''}.`
    );
  ...
  const numbersToRemove = localPhoneNumbers.filter(
    (number) => !remotePhoneNumbers.includes(number)
  );
  ...
  console.log(
    chalk.yellow(
      `Found ${numbersToRemove.length} ${providerName} phone numbers to remove from local configuration:`
    )
  );
  ...
  for (const number of numbersToRemove) {
    ...
    for (const configKey of configKeys) {
      if (
        config[configKey] &&
        config[configKey].phoneNumbers &&
        config[configKey].phoneNumbers[number]
      ) {
  ...
  if (removedCount > 0) {
    console.log(
      chalk.green(
        `Removed ${removedCount} ${providerName} phone numbers${domainFilter ? ` from domain ${domainFilter}` : ''}.`
      )
    );
  } else {
