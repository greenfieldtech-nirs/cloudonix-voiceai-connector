...
function displayDomainConfig(domainName, domainConfig) {
  ...
  providers.forEach((providerName) => {
    if (domainConfig[providerName.toLowerCase()]) {
      console.log(
        chalk.cyan(`${providerName} Trunk Credential ID:`),
      ...
      try {
        ...
        loadingInterval = setInterval(() => {
          process.stdout.write(
            `\r${chalk.cyan(loadingChars[i])} Fetching ${providerName} phone numbers...`
          );
        ...
        if (providerName === 'VAPI') {
              ...
              try {
                ...
                if (details.credentialId) {
                  try {
                    ...
                    process.stdout.write(
                      `\r${chalk.cyan(loadingChars[i % loadingChars.length])} Fetching credential details for ${details.number}...`
                    );
                    credentialDetails = await vapiService.getCredentialDetails(
                      details.credentialId
                    );
                  } catch (credError) {
                    console.error(
                      chalk.yellow(
                        `    Unable to fetch credential details for ID ${details.credentialId}: ${credError.message}`
                      )
                    );
                  }
                }
                ...
                console.error(
                  chalk.red(`    Error fetching details for number ${num.number}: ${error.message}`)
                );
          ...
          if (phoneNumbersData && Array.isArray(phoneNumbersData)) {
            remoteNumbers = phoneNumbersData.map((num) => ({
              ...
              lastModified:
                num.last_modification_timestamp || num.lastModificationTimestamp
                  ? new Date(
                      parseInt(num.last_modification_timestamp || num.lastModificationTimestamp) *
                        1000
                    ).toLocaleString()
                  : null,
            }));
          ...
          try {
            const elevenLabsProvider = new ElevenLabsAgentProvider(
            ...
            if (process.argv.includes('--debug')) {
              ...
              console.log(
                'Phone Numbers Count:',
                Array.isArray(phoneNumbersData) ? phoneNumbersData.length : 'Not an array'
              );
              console.log('===================================\n');
            }
            ...
            if (phoneNumbersData && Array.isArray(phoneNumbersData)) {
              ...
              if (
                providerConfig.phoneNumbers &&
                Object.keys(providerConfig.phoneNumbers).length > 0
              ) {
                remoteNumbers = Object.entries(providerConfig.phoneNumbers).map(
                  ([number, details]) => ({
          ...
          remoteNumbers.forEach((num) => {
            ...
            if (providerName === 'VAPI') {
              ...
              if (num.termination_uri) {
                ...
                const formattedNumber = num.number.startsWith('+')
            ...
            if (providerName === 'VAPI') {
              ...
              if (num.error) {
                ...
                if (num.credentialId) {
                  ...
                  if (num.credentialDetails) {
                    ...
                    console.log(
                      chalk.yellow(`      Provider: ${num.credentialDetails.provider || 'N/A'}`)
                    );

                    if (
                      num.credentialDetails.gateways &&
                      num.credentialDetails.gateways.length > 0
                    ) {
                ...
                if (num.createdAt) {
                  console.log(
                    chalk.yellow(`    Created: ${new Date(num.createdAt).toLocaleString()}`)
                  );
                }

                if (num.updatedAt) {
                  console.log(
                    chalk.yellow(`    Updated: ${new Date(num.updatedAt).toLocaleString()}`)
                  );
                }
                ...
                if (num.fallbackDestination) {
                  ...
                  if (num.fallbackDestination.callerId) {
                    console.log(
                      chalk.yellow(`      Caller ID: ${num.fallbackDestination.callerId}`)
                    );
                  }

                  if (num.fallbackDestination.description) {
                    console.log(
                      chalk.yellow(`      Description: ${num.fallbackDestination.description}`)
                    );
                  }
                ...
                if (num.hooks && num.hooks.length > 0) {
                  ...
                  num.hooks.forEach((hook, index) => {
                    ...
                    if (hook.do && hook.do.length > 0) {
                      console.log(
                        chalk.yellow(`        Actions: ${hook.do.map((a) => a.type).join(', ')}`)
                      );
                    }
                  });
                }
                ...
                if (num.server && num.server.url) {
                  ...
                  if (num.server.timeoutSeconds) {
                    console.log(
                      chalk.yellow(`      Timeout: ${num.server.timeoutSeconds} seconds`)
                    );
                  }
                  ...
                  if (num.server.headers) {
                    console.log(
                      chalk.yellow(
                        `      Custom Headers: ${Object.keys(num.server.headers).length}`
                      )
                    );
                  }
                }
              ...
              Object.entries(num).forEach(([key, value]) => {
                ...
                if (
                  ...
                  if (typeof value === 'object') {
                    console.log(
                      chalk.yellow(
                        `    ${key.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}: ${JSON.stringify(value)}`
                      )
                    );
                  } else {
                    console.log(
              ...
              if (num.error) {
                ...
                if (num.created_at && num.created_at !== 'N/A') {
                  ...
                  try {
                    if (
                      typeof num.created_at === 'number' ||
                      !isNaN(new Date(num.created_at).getTime())
                    ) {
                ...
                if (
                  num.termination_uri &&
                  num.termination_uri !== 'N/A' &&
                  num.termination_uri !== sipUri
                ) {
        ...
        console.error(
          chalk.red(`  Error fetching remote ${providerName} phone numbers: ${error.message}`)
        );
      }
    }
  } else {
...
function displayPhoneNumbers(providerPhoneNumbers) {
  const hasPhoneNumbers = Object.values(providerPhoneNumbers).some(
    (numbers) => numbers && Object.keys(numbers).length > 0
  );
  ...
  if (hasPhoneNumbers) {
    ...
    Object.entries(providerPhoneNumbers).forEach(([providerName, phoneNumbers]) => {
      if (phoneNumbers && Object.keys(phoneNumbers).length > 0) {
        ...
        Object.entries(phoneNumbers).forEach(([number, _config]) => {
          let sipUri = '';
          if (providerName === 'VAPI') {
