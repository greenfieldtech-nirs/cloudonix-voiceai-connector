...
async function addNumberCommand(options) {
  const { domain, provider, number } = options;
  ...
  try {
    ...
    if (provider.toLowerCase() === 'vapi') {
      result = await apiService.addByoPhoneNumber(
        domain,
        number,
        domainConfig.vapi.trunkCredentialId
      );
      ...
      if (!domainConfig.inboundSipUri) {
        throw new Error(
          `No inbound SIP URI found for domain ${domain}. Please configure it first.`
        );
      }
    ...
    console.log(
      chalk.cyan('SIP URI:'),
      chalk.yellow(domainConfig[provider.toLowerCase()].phoneNumbers[number].sipUri)
    );
